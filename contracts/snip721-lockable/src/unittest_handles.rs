...
mod tests {
    ...
    use crate::subscriber_msg;
    use crate::token::{Extension, Metadata, Token, Trait};
    ...
    fn extract_messages(hr: HandleResponse) -> Vec<CosmosMsg> {
    ...
    fn test_batch_mint() -> StdResult<()> {
        ...
        let other_mints = vec![Mint {
            token_id: Some("sillyToken".to_string()),
            owner: None,
        ...
        assert!(Inventory::owns(&deps.storage, &alice_raw, 3).unwrap());
        assert!(Inventory::owns(&deps.storage, &admin_raw, 2).unwrap());
        assert!(Inventory::owns(&deps.storage, &admin_raw, 4).unwrap());
        // verify mint tx was logged
        let (txs, total) = get_txs(&deps.api, &deps.storage, &admin_raw, 0, 4).unwrap();
        assert_eq!(total, 5);
        assert_eq!(txs[0].token_id, "4".to_string());
        assert_eq!(
            txs[0].action,
            TxAction::Mint {
                minter: admin.clone(),
    ...
    fn test_mint() {
        ...
        let handle_msg = HandleMsg::MintNft {
            token_id: Some("testtesttest".to_string()),
    ...
    fn test_lock() -> StdResult<()> {
        ...
        // test token does not exist when supply is public
        let handle_msg = HandleMsg::LockNft {
            token_id: "lockableNFT".to_string(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("alice", &[]), handle_msg);
        let error = extract_error_msg(handle_result);
        assert_eq!(error.as_str(), "Token ID: lockableNFT not found");
        ...
        // test token does not exist when supply is private
        let handle_msg = HandleMsg::Reveal {
            token_id: "lockableNFT2".to_string(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("alice", &[]), handle_msg);
        let error = extract_error_msg(handle_result);
        assert_eq!(error.as_str(), "You do not own token lockableNFT2");
        ...
        let handle_msg = HandleMsg::MintNft {
            token_id: Some("lockableNFT3".to_string()),
            owner: Some(HumanAddr("alice".to_string())),
            private_metadata: Some(Metadata {
                token_uri: None,
                extension: Some(Extension {
                    name: Some("MyLockableNFT3".to_string()),
                    description: Some("locked nft test".to_string()),
                    image: Some("uri".to_string()),
                    ..Extension::default()
                }),
            }),
        ...
        // test lock when status prevents it
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopLocks,
            padding: None,
        };
        let _handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);

        let handle_msg = HandleMsg::LockNft {
            token_id: "lockableNFT3".to_string(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("alice", &[]), handle_msg);
        let error = extract_error_msg(handle_result);
        ...
        // test unlock when status prevents it
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopUnlocks,
            padding: None,
        };
        let _handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);

        let handle_msg = HandleMsg::UnlockNft {
            token_id: "lockableNFT3".to_string(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("alice", &[]), handle_msg);
        ...
        // test someone other than owner tries to lock
        let handle_msg = HandleMsg::LockNft {
            token_id: "lockableNFT3".to_string(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        let error = extract_error_msg(handle_result);
        assert_eq!(error.as_str(), "You do not own token lockableNFT3");

        // test trying to lock token without public metadata
        let handle_msg = HandleMsg::LockNft {
            token_id: "lockableNFT3".to_string(),
            padding: None,
        };
        ...
        assert_eq!(
            error.as_str(),
            "locking a token without the 'multiplier' trait is not currently supported"
        );

        // test trying to lock token without extensions in public metadata
        let handle_msg = HandleMsg::SetMetadata {
            token_id: "lockableNFT3".to_string(),
            public_metadata: Some(Metadata {
                token_uri: None,
        ...
        // test trying to lock token without attributes in public metadata
        let handle_msg = HandleMsg::SetMetadata {
            token_id: "lockableNFT3".to_string(),
            public_metadata: Some(Metadata {
                token_uri: None,
        ...
        // test trying to lock token without "multiplier" trait in attributes
        let handle_msg = HandleMsg::SetMetadata {
            token_id: "lockableNFT3".to_string(),
            public_metadata: Some(Metadata {
                token_uri: None,
                extension: Some(Extension {
                    ...
                    attributes: Some(vec![Trait {
                        display_type: None,
                        trait_type: Some(String::from("hair color")),
                        value: "101000".to_string(),
                        max_value: None,
                    }]),
                    background_color: None,
                    animation_url: None,
        ...
        // test trying to lock token whose "multiplier" trait is not an int
        let handle_msg = HandleMsg::SetMetadata {
            token_id: "lockableNFT3".to_string(),
            public_metadata: Some(Metadata {
                token_uri: None,
                extension: Some(Extension {
                    ...
                    attributes: Some(vec![Trait {
                        display_type: None,
                        trait_type: Some(String::from("multiplier")),
        ...
        assert_eq!(
            error.as_str(),
            "could not parse 'multiplier' trait value as an integer"
        );

        // sanity check, lock token
        let handle_msg = HandleMsg::SetMetadata {
            token_id: "lockableNFT3".to_string(),
            public_metadata: Some(Metadata {
                token_uri: None,
        ...
        let handle_response = handle(&mut deps, mock_env("alice", &[]), handle_msg)?;
        let messages = extract_messages(handle_response);
        assert!(messages.len() == 0); //no subscribers

        let map2idx = ReadonlyPrefixedStorage::new(PREFIX_MAP_TO_INDEX, &deps.storage);
        let index: u32 = load(&map2idx, "lockableNFT3".as_bytes()).unwrap();
        let token_key = index.to_le_bytes();
        let map2id = ReadonlyPrefixedStorage::new(PREFIX_MAP_TO_ID, &deps.storage);
        let id: String = load(&map2id, &token_key).unwrap();
        assert_eq!("lockableNFT3".to_string(), id);
        let info_store = ReadonlyPrefixedStorage::new(PREFIX_INFOS, &deps.storage);
        let nft_key = index.to_le_bytes();
        let token: Token = json_load(&info_store, &nft_key).unwrap();
        assert_eq!(
            token.owner,
            deps.api.canonical_address(&HumanAddr::from("alice"))?
        );
        assert!(token.unwrapped);
        assert!(token.locked);
        ...
        // test locked token is not transferable
        ...
        assert_eq!(
            ...
            "Attempting to transfer a locked token. ID: lockableNFT3. It must be unlocked first."
        );
        ...
        // test locked token is not transferable even by whitelisted addresses
        let handle_msg = HandleMsg::SetWhitelistedApproval {
            address: HumanAddr("michael".to_string()),
            token_id: Some("lockableNFT3".to_string()),
        ...
        let handle_result = handle(&mut deps, mock_env("michael", &[]), handle_msg);
        assert!(handle_result.is_err());
        let error_message = extract_error_msg(handle_result);
        assert_eq!(
            error_message.as_str(),
        ...
        // test locked token is transferable again
        let handle_msg = HandleMsg::TransferNft {
            recipient: HumanAddr("bob".to_string()),
            token_id: "lockableNFT3".to_string(),
            memo: None,
        ...
        assert_eq!(
            token.owner,
            deps.api.canonical_address(&HumanAddr::from("bob"))?
        );
        assert!(token.unwrapped);
        assert!(!token.locked);

        // test that unlocking a token that is already unlocked does nothing
        let handle_msg = HandleMsg::UnlockNft {
            token_id: "lockableNFT3".to_string(),
            padding: None,
        };
        ...
        // sanity check with subscribers
        let handle_msg = HandleMsg::AddSubs {
            contracts: vec![
                Contract {
                ...
                Contract {
                    address: HumanAddr::from("Dan"),
                    hash: "danhash".to_string(),
                },
            ],
            padding: None,
        };
        ...
        let handle_response = handle(&mut deps, mock_env("bob", &[]), handle_msg)?;
        let messages = extract_messages(handle_response);
        ...
        assert_eq!(
            ...
            vec![
                CosmosMsg::Wasm(WasmMsg::Execute {
                    contract_addr: HumanAddr::from("charlie"),
                    callback_code_hash: "hashash".to_string(),
                    msg: to_binary(&subscriber_msg::HandleMsg::ApplyMultiplier {
                        to: HumanAddr::from("bob"),
                        multiplier: 101000,
                        item_id: "lockableNFT3".to_string()
                    })?,
                    send: vec![],
                }),
                CosmosMsg::Wasm(WasmMsg::Execute {
                    contract_addr: HumanAddr::from("Dan"),
                    callback_code_hash: "danhash".to_string(),
                    msg: to_binary(&subscriber_msg::HandleMsg::ApplyMultiplier {
                        to: HumanAddr::from("bob"),
                        multiplier: 101000,
                        item_id: "lockableNFT3".to_string()
        ...
        let query_answer: QueryAnswer = from_binary(&query(
            &deps,
            QueryMsg::IsLocked {
                token_id: "lockableNFT3".to_string(),
                viewing_key: "bob_key".to_string(),
            },
        )?)?;
        match query_answer {
            QueryAnswer::IsLocked { token_is_locked } => assert!(token_is_locked),
            _ => panic!("wrong query answer type, expected 'IsLocked'"),
        }

        // check that it notifies the subs again if the token's re-locked
        let handle_msg = HandleMsg::LockNft {
            token_id: "lockableNFT3".to_string(),
            padding: None,
        };
        ...
        // test unlocking with subscribers
        let handle_msg = HandleMsg::UnlockNft {
            token_id: "lockableNFT3".to_string(),
            padding: None,
        };
        ...
        assert_eq!(
            ...
            vec![
                CosmosMsg::Wasm(WasmMsg::Execute {
                    contract_addr: HumanAddr::from("charlie"),
                    callback_code_hash: "hashash".to_string(),
                    msg: to_binary(&subscriber_msg::HandleMsg::DropMultiplier {
        ...
        // test isLocked query now that it's unlocked
        let query_answer: QueryAnswer = from_binary(&query(
            &deps,
            QueryMsg::IsLocked {
                token_id: "lockableNFT3".to_string(),
    ...
    #[test]
    fn test_add_remove_subscribers() -> StdResult<()> {
        let (init_result, mut deps) = init_helper_default();
        assert!(
            init_result.is_ok(),
            "Init failed: {}",
        ...
        // test add subscribers when status prevents it
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::StopLocks,
            padding: None,
        };
        ...
        let subs = vec![
            ...
            Contract {
                ...
                hash: "hash-is-not-validated".to_string(),
            },
            Contract {
        ...
        let sub_addresses: Vec<HumanAddr> =
            subs.iter().take(2).map(|c| c.address.clone()).collect();

        let handle_msg = HandleMsg::AddSubs {
            contracts: subs.clone(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        let error = extract_error_msg(handle_result);
        ...
        // test remove subscribers when status prevents it
        let handle_msg = HandleMsg::RemoveSubs {
            contracts: sub_addresses.clone(),
            padding: None,
        };
        let handle_result = handle(&mut deps, mock_env("admin", &[]), handle_msg);
        let error = extract_error_msg(handle_result);
        ...
        // test non admin trying to add subscribers
        let handle_msg = HandleMsg::SetContractStatus {
            level: ContractStatus::Normal,
            padding: None,
        };
        ...
        // test non admin trying to remove subscribers
        let handle_msg = HandleMsg::RemoveSubs {
            contracts: sub_addresses.clone(),
            padding: None,
        };
        ...
        // verify the new subscribers were added
        let query_answer: QueryAnswer = from_binary(&query(&deps, QueryMsg::Subscribers {})?)?;
        match query_answer {
            QueryAnswer::Subscribers { contracts } => assert_eq!(contracts, subs),
            _ => panic!("wrong query answer type, expected 'Subscribers'"),
        }
        ...
        // verify the subscribers where removed
        let query_answer: QueryAnswer = from_binary(&query(&deps, QueryMsg::Subscribers {})?)?;
        match query_answer {
            QueryAnswer::Subscribers { contracts } => {
                assert_eq!(
                    contracts,
                    vec![Contract {
                        address: HumanAddr::from("charlie"),
